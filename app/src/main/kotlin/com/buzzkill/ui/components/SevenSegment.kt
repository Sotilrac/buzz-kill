...
private val DigitMap: Map<Char, BooleanArray> = mapOf(
...
@Composable
private fun ColonDot(digitHeight: Dp, color: Color) {
...
private fun DrawScope.drawSevenSegment(segments: BooleanArray, lit: Color, unlit: Color) {
    val w = size.width
    val h = size.height
    ...
    fun horiz(yCenter: Float): Path = Path().apply {
        val y0 = yCenter - t / 2f
        val y1 = yCenter + t / 2f
        moveTo(pad + t / 2f, y0)
    ...
    // Vertical segments
    fun vert(xCenter: Float, yTop: Float, yBot: Float): Path = Path().apply {
        val x0 = xCenter - t / 2f
        val x1 = xCenter + t / 2f
        moveTo(x0, yTop + t / 2f)
        lineTo(xCenter, yTop)
